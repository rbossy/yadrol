...
class ValueEqual extends Validate {
    ...
    check(value) {
	return ValueComparator.equal(value, this.value);
    }

    display() {
	return ValueConverter.valueString(new Scope(), this.value);
    }
}
...
class Test {
	...
	if (result.error) {
	    ...
	    rcol = ValueConverter.valueString(globalScope, result.result);
	}
...
class TestConversion extends Test {
	...
	super('convert: ' + ValueConverter.valueString(new Scope(), value) + ' -> ' + type, new ValueEqual(expect));
	this.value = value;
	this.type = type;
    }

    getResult() {
	return ValueConverter.convert(new Scope(), this.value, this.type);
    }
}
...
class TestLexer extends Test {
    ...
    getResult() {
	var result = [];
	yadrolParser.lexer.setInput(this.input);
	while (true) {
...
class TestParse extends Test {
    ...
    parse() {
	return yadrolParser.parseExpressions('test.js', this.input, new RecordLogger(10000));
    }
}
...
$(document).ready(function() {
	...
	new TestLexer('"cool"', ["STR_START", "STR_CONST", "STR_END"]),
	...
	new TestParseEvaluate('repeat 42 if 0 == 0', 'native', [42, 42]),
	new TestParseEvaluate('repeat (n = n + 1; n) while n < 4', 'native', [1, 2, 3, 4]),
	new TestParseEvaluate('repeat (n = n + 1; n) while n < 4 limit 2', 'native', [1, 2, 3]),
	new TestParseEvaluate('while n < 4 repeat (n = n + 1; n)', 'native', [1, 2, 3, 4]),
	new TestParseEvaluate('while n < 4 repeat (n = n + 1; n) limit 2', 'native', [1, 2, 3]),
    ]).table();
