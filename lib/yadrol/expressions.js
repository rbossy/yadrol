...
class Expression {
	...
	throw 'unimplemented nativeEvaluator(scope)';
    }
...
class Sequence extends BinaryOperator {
    constructor(location, left, right) {
	super(location, Sequence.OPERATOR.precedence, right.nativeType, 'native', 'native', left, right);
    }
...
class Subscript extends Expression {
    ...
    compute(scope) {
	this._compute(this.container.evaluate(scope), this.subscript.evaluate(scope));
    }
    
    _compute(container, subscript) {
	var container = this.container.evaluate(scope);
	var subscript = this.subscript.evaluate(scope);
	...
	case 'list':
	case 'map': {
	    return subscript.map(function(i) { this._compute(container, i); });
	}
	case 'function': {
	    throw 'invalid subscript ' + subscript;
	}
