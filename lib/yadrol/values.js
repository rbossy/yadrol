...
class YadrolFunction {
	...
	var scope = _createScope(posArgs, namedArgs);
	return body.evaluate(scope, type);
    }
	...
	var posArgsIt = posArgs.values();
	for (var name of this.args.keys()) {
	    var n = posArgsIt.next();
	    if (n.done) {
		break;
	    }
	    variables.set(name, n.value);
	}
...
var valueType = function(value) {
    ...
    throw 'unhandled data type: ' + value;
}
...
var _convertToContainerConstructor = function(scope, value) {
    return new ContainerConstructor(null, value.map(function(v) { return _convertToExpression(scope, v); }), valueType(value));
}
