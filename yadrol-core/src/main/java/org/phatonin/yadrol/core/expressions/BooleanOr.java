...
public class BooleanOr extends AbstractBooleanExpression {
	...
	public BooleanOr(List<Expression> operands) {
	...
	public AbstractBooleanExpression reduce() throws EvaluationException {
		...
		for (Expression e : this.operands) {
			if (e.isPureConstant() && e.evaluateBoolean(null, null)) {
				operands.add(new BooleanConstant(e.getLocation(), true));
				break;
			}
			operands.add(e.reduce());
		}
		if (operands.isEmpty()) {
			return new BooleanConstant(getLocation(), false);
		}
		if (operands.size() == 1) {
	...
	public Expression substituteVariables(Scope scope) {
		return new BooleanOr(substituteVariables(operands, scope));
	}
